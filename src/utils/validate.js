...
export const signupValidate = (refs,isChecked1,isChecked2) => {
    const refsEntries = Object.entries(refs);
    const msgs = {'idRef':'아이디','passwordRef':'비밀번호','cpwdRef':'비밀번호확인', 'usernameRef':'이름',
        'phoneRef':'전화번호','addressRef':'주소','emailRef':'이메일'
    };
    for(const item of refsEntries){  //itme = ['id':~~~]
        const name = item[0];
        const ref = item[1];
